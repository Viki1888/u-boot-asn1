...
#ifndef CONFIG_TARGET_LIGHT_FPGA_FM_C910
#if defined   ( CONFIG_TARGET_LIGHT_FM_C910_VAL_SV_B)
...
static void light_iopin_init(void)
{
	...
	light_pin_mux(GPIO1_21,2);
	light_pin_mux(GPIO1_22,2);
	light_pin_mux(GPIO1_23,2);
	light_pin_mux(GPIO1_24,2);
	light_pin_mux(GPIO1_25,2);
	light_pin_mux(GPIO1_26,2);
	light_pin_mux(GPIO1_27,2);
	light_pin_mux(GPIO1_28,2);
	light_pin_mux(GPIO1_29,2);
	light_pin_mux(GPIO1_30,2);
...
#elif defined ( CONFIG_LIGHT_BOARD_EVB)
static void light_iopin_init(void)
{
	/*AON-PADMUX*/
...
static void sec_upgrade_thread(void)
{
	...
	if (sec_upgrade_flag == TF_SEC_UPGRADE_FLAG) {
		unsigned long temp_addr=0x10000000;
		...
		run_command("ext4load mmc 0:4 0x10000000 trust_firmware.bin", 0);
		...
		run_command("ext4write mmc 0:3 0x10000000 trust_firmware.bin 850000", 0);
